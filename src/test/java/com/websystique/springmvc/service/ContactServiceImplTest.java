...
import com.websystique.springmvc.service.impl.ContactServiceImpl;
...
public class ContactServiceImplTest {
	...
	@InjectMocks
	ContactServiceImpl employeeService;
	...
	public void findById(){
		...
		when(dao.selectContact(anyInt())).thenReturn(emp);
		Assert.assertEquals(employeeService.getContact(emp.getId()),emp);
	}

	@Test
	public void saveEmployee(){
		doNothing().when(dao).saveEmployee(any(Employee.class));
		employeeService.addContact(any(Employee.class));
	...
	public void deleteEmployeeBySsn(){
		...
		employeeService.deleteContact(anyString());
		verify(dao, atLeastOnce()).deleteEmployeeBySsn(anyString());
	}
	
	@Test
	public void findAllEmployees(){
		when(dao.selectAllContacts()).thenReturn(employees);
		Assert.assertEquals(employeeService.getAllContacts(), employees);
	}
	
	@Test
	public void findEmployeeBySsn(){
		Employee emp = employees.get(0);
		when(dao.selectContact(anyString())).thenReturn(emp);
		Assert.assertEquals(employeeService.getContact(anyString()), emp);
	}
